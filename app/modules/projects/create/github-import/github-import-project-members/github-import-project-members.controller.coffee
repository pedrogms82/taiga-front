...
#
# File: github-import-project-members.controller.coffee
###
...
class GithubImportProjectMembersController
    ...
    confirmUser: (githubUser, taigaUser) ->
        @.selectImportUserLightbox = false

        user = Immutable.Map()

        user = user.set('githubUser', githubUser)
    ...
    getSelectedMember: (member) ->
        return @.selectedUsers.find (it) ->
            return it.getIn(['githubUser', 'id']) == member.get('id')
    ...
    submit: () ->
        ...
        @.selectedUsers.map (it) ->
            users = users.set(it.getIn(['githubUser', 'id']), it.getIn(['taigaUser', 'id']))

        @.onSubmit({users: users})

angular.module('taigaProjects').controller('GithubImportProjectMembersCtrl', GithubImportProjectMembersController)
