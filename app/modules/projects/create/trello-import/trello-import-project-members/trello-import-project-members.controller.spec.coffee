...
describe "TrelloImportProjectMembersCtrl", ->
    $provide = null
    $controller = null
    mocks = {}
    ...
    it "search user", () ->
        ctrl = $controller("TrelloImportProjectMembersCtrl")
    ...
    it "confirm user", () ->
        ...
        ctrl.confirmUser('trello-user', 'taiga-user')

        expect(ctrl.selectedUsers.size).to.be.equal(1)

        expect(ctrl.selectedUsers.get(0).get('trelloUser')).to.be.equal('trello-user')
    ...
    it "get a selected member", () ->
        ...
        expect(user.getIn(['trelloUser', 'id'])).to.be.equal(3)
